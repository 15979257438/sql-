...
interface DraftData { images: string[]; title: string; description: string; price: string; originalPrice: string; category: Category | ''; condition: Condition; tradeMethod: TradeMethod; location: string; }
...
export default function PublishPage() {
  ...
  useEffect(() => { try { const draft = storage.get<DraftData>(DRAFT_KEY); if (draft) setHasDraft(true); } catch { /* ignore */ } }, []);

  const restoreDraft = () => {
    try { const draft = storage.get<DraftData>(DRAFT_KEY); if (!draft) return; setImages(draft.images || []); setTitle(draft.title || ''); setDescription(draft.description || ''); setPrice(draft.price || ''); setOriginalPrice(draft.originalPrice || ''); setCategory(draft.category || ''); setCondition(draft.condition || 'like_new'); setTradeMethod(draft.tradeMethod || 'both'); setLocation(draft.location || ''); setHasDraft(false); showToast('已恢复上次草稿', 'success'); } catch { /* ignore */ }
  };
  const clearDraft = () => { storage.remove(DRAFT_KEY); };
  const saveDraft = useCallback(() => { const draft: DraftData = { images, title, description, price, originalPrice, category, condition, tradeMethod, location }; storage.set(DRAFT_KEY, draft); }, [images, title, description, price, originalPrice, category, condition, tradeMethod, location]);
  useEffect(() => { const timer = setInterval(saveDraft, 10000); return () => { clearInterval(timer); saveDraft(); }; }, [saveDraft]);
  ...
  const handleSubmit = async () => { const error = validate(); if (error) { showToast(error, 'error'); return; } setSubmitting(true); try { const data = { title: title.trim(), description: description.trim(), price: Math.round(Number(price) * 100), originalPrice: originalPrice ? Math.round(Number(originalPrice) * 100) : undefined, category: category as Category, condition, images, tradeMethod, location: location.trim(), sellerId: user?.id || '' }; const product = await productService.create(data); clearDraft(); showToast('发布成功', 'success'); goProduct(product.id); } catch (e: any) { showToast(e.message || '操作失败', 'error'); } finally { setSubmitting(false); } };
  ...
  return (
    <View style={{ minHeight: '100vh', backgroundColor: '#F5F7FA' }}>
      <NavBar title="发布商品" />
      <ScrollView scrollY style={{ padding: '32rpx', paddingBottom: 'calc(280rpx + env(safe-area-inset-bottom))' }}>
        {hasDraft && <View style={{ ...cardStyle, marginBottom: '16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}><Text style={{ fontSize: '28rpx', color: '#666666' }}>检测到有未提交的草稿</Text><View onClick={restoreDraft} style={{ padding: '4px 8px' }}><Text style={{ fontSize: '28rpx', color: '#2B8CFF', fontWeight: 500 }}>恢复</Text></View></View>}
        <View style={{ ...cardStyle, marginBottom: '16px' }}>
          <Text style={{ ...labelStyle, marginBottom: '12px' }}>商品图片 <Text style={{ color: '#666666', fontWeight: 'normal' }}>({images.length}/9)</Text></Text>
          {uploadProgress && <View style={{ marginBottom: '16px' }}><View style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}><Text style={{ fontSize: '24rpx', color: '#2B8CFF' }}>正在上传 {uploadProgress.current}/{uploadProgress.total}</Text><Text style={{ fontSize: '24rpx', color: '#666666' }}>{Math.round(uploadProgress.current / uploadProgress.total * 100)}%</Text></View><View style={{ width: '100%', height: '8px', backgroundColor: '#E8F4FF', borderRadius: '4px', overflow: 'hidden' }}><View style={{ width: `${(uploadProgress.current / uploadProgress.total) * 100}%`, height: '100%', background: 'linear-gradient(90deg, #2B8CFF 0%, #6C5CE7 100%)', borderRadius: '4px', transition: 'width 0.3s ease' }} /></View></View>}
          <View style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '24rpx' }}>
            ...
            {images.length < 9 && <Button onClick={handleImageAdd} style={{ aspectRatio: '1', border: '2px dashed #2B8CFF', borderRadius: '24rpx', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#2B8CFF', backgroundColor: 'rgba(43, 140, 255, 0.05)', padding: '0', transition: 'all 0.3s ease' }}><Text style={{ fontSize: '48rpx', color: '#2B8CFF' }}>＋</Text><Text style={{ fontSize: '24rpx', marginTop: '4px', color: '#2B8CFF' }}>添加图片</Text></Button>}
          </View>
        </View>
        <View style={{ ...cardStyle, marginBottom: '16px' }}><Text style={{ ...labelStyle, marginBottom: '8px' }}>标题</Text><Input type="text" value={title} onInput={e => { setTitle(e.detail.value); validateField('title', e.detail.value); }} onBlur={() => validateField('title', title)} placeholder="请输入商品标题" maxlength={50} style={getInputStyle('title')} /><View style={{ display: 'flex', justifyContent: 'space-between', marginTop: '8px' }}><Text style={{ fontSize: '24rpx', color: validation.title.valid ? '#666666' : '#FF4D4F' }}>{validation.title.touched && !validation.title.valid && validation.title.message}</Text><Text style={{ fontSize: '24rpx', color: '#666666' }}>{title.length}/50</Text></View></View>
        <View style={{ ...cardStyle, marginBottom: '16px' }}><Text style={{ ...labelStyle, marginBottom: '8px' }}>描述</Text><Textarea value={description} onInput={e => { setDescription(e.detail.value); validateField('description', e.detail.value); }} placeholder="描述商品成色、使用情况等" maxlength={500} style={{ width: '100%', height: '192rpx', padding: '24rpx 32rpx', backgroundColor: '#F5F7FA', border: `1px solid ${validation.description.valid ? '#EEEEEE' : '#FF4D4F'}`, borderRadius: '24rpx', fontSize: '28rpx', boxSizing: 'border-box' }} /><View style={{ display: 'flex', justifyContent: 'space-between', marginTop: '8px' }}><Text style={{ fontSize: '24rpx', color: '#FF4D4F' }}>{validation.description.touched && !validation.description.valid && validation.description.message}</Text><Text style={{ fontSize: '24rpx', color: '#666666' }}>{description.length}/500</Text></View></View>
        <View style={{ ...cardStyle, marginBottom: '16px' }}><View style={{ display: 'flex', gap: '24rpx' }}><View style={{ flex: 1 }}><Text style={{ ...labelStyle, marginBottom: '8px' }}>售价</Text><View style={{ display: 'flex', alignItems: 'center', backgroundColor: '#F5F7FA', border: `1px solid ${validation.price.valid ? '#EEEEEE' : '#FF4D4F'}`, borderRadius: '24rpx', height: '88rpx', paddingLeft: '32rpx', paddingRight: '32rpx' }}><Text style={{ color: '#666666', fontSize: '28rpx' }}></Text><Input type="digit" value={price} onInput={e => { setPrice(e.detail.value); validateField('price', e.detail.value); }} placeholder="0" style={{ flex: 1, paddingLeft: '8px', backgroundColor: 'transparent', fontSize: '28rpx' }} /></View>{price && Number(price) > 0 && <Text style={{ fontSize: '24rpx', color: '#2B8CFF', marginTop: '8px' }}>约 {Number(price).toFixed(2)}</Text>}</View><View style={{ flex: 1 }}><Text style={{ ...labelStyle, marginBottom: '8px' }}>原价 <Text style={{ color: '#666666', fontWeight: 'normal' }}>(选填)</Text></Text><View style={{ display: 'flex', alignItems: 'center', backgroundColor: '#F5F7FA', border: '1px solid #EEEEEE', borderRadius: '24rpx', height: '88rpx', paddingLeft: '32rpx', paddingRight: '32rpx' }}><Text style={{ color: '#666666', fontSize: '28rpx' }}></Text><Input type="digit" value={originalPrice} onInput={e => setOriginalPrice(e.detail.value)} placeholder="0" style={{ flex: 1, paddingLeft: '8px', backgroundColor: 'transparent', fontSize: '28rpx' }} /></View>{originalPrice && Number(originalPrice) > 0 && price && Number(price) > 0 && <Text style={{ fontSize: '24rpx', color: '#52C41A', marginTop: '8px' }}>约 {Math.round((1 - Number(price) / Number(originalPrice)) * 100)}% 折扣</Text>}</View></View></View>
        <View style={{ ...cardStyle, marginBottom: '16px' }}><Text style={{ ...labelStyle, marginBottom: '12px' }}>分类</Text><View style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>{CATEGORIES.map(cat => <Button key={cat.value} onClick={() => setCategory(cat.value)} style={{ padding: '16rpx 32rpx', borderRadius: '9999px', fontSize: '28rpx', background: category === cat.value ? 'linear-gradient(135deg, #2B8CFF 0%, #6C5CE7 100%)' : '#F5F7FA', color: category === cat.value ? '#ffffff' : '#666666', border: 'none', lineHeight: '1.5' }}>{cat.label}</Button>)}</View></View>
        <View style={{ ...cardStyle, marginBottom: '16px' }}><Text style={{ ...labelStyle, marginBottom: '12px' }}>成色</Text><View style={{ display: 'flex', gap: '8px' }}>{CONDITIONS.map(c => <Button key={c.value} onClick={() => setCondition(c.value)} style={{ flex: 1, padding: '20rpx 0', borderRadius: '24rpx', fontSize: '28rpx', background: condition === c.value ? 'linear-gradient(135deg, #2B8CFF 0%, #6C5CE7 100%)' : '#F5F7FA', color: condition === c.value ? '#ffffff' : '#666666', border: 'none', lineHeight: '1.5' }}>{c.label}</Button>)}</View></View>
        <View style={{ ...cardStyle, marginBottom: '16px' }}><Text style={{ ...labelStyle, marginBottom: '12px' }}>交易方式</Text><View style={{ display: 'flex', gap: '8px' }}>{TRADE_METHODS.map(m => <Button key={m.value} onClick={() => setTradeMethod(m.value)} style={{ flex: 1, padding: '20rpx 0', borderRadius: '24rpx', fontSize: '28rpx', background: tradeMethod === m.value ? 'linear-gradient(135deg, #2B8CFF 0%, #6C5CE7 100%)' : '#F5F7FA', color: tradeMethod === m.value ? '#ffffff' : '#666666', border: 'none', lineHeight: '1.5' }}>{m.label}</Button>)}</View></View>
        {showLocation && <View style={{ ...cardStyle, marginBottom: '16px' }}><Text style={{ ...labelStyle, marginBottom: '8px' }}>交易地点</Text><Input type="text" value={location} onInput={e => setLocation(e.detail.value)} placeholder="如：紫荆公寓3号楼" maxlength={50} style={{ width: '100%', height: '88rpx', paddingLeft: '32rpx', paddingRight: '32rpx', backgroundColor: '#F5F7FA', border: '1px solid #EEEEEE', borderRadius: '24rpx', fontSize: '28rpx' }} /></View>}
      </ScrollView>
      <View style={{ position: 'fixed', bottom: 'calc(120rpx + env(safe-area-inset-bottom))', left: 0, right: 0, backgroundColor: '#ffffff', borderTop: '1px solid #EEEEEE', padding: '24rpx 32rpx', zIndex: 40, paddingBottom: '12px' }}>
        <Button onClick={handleSubmit} disabled={submitting} style={{ width: '100%', height: '96rpx', background: submitting ? '#CCCCCC' : 'linear-gradient(135deg, #2B8CFF 0%, #6C5CE7 100%)', color: '#ffffff', borderRadius: '32rpx', fontWeight: 600, fontSize: '32rpx', border: 'none', boxShadow: submitting ? 'none' : '0 4px 15px rgba(43, 140, 255, 0.4)', lineHeight: '1.5' }}>{submitting ? '提交中...' : '立即发布'}</Button>
      </View>
    </View>
  );
}
